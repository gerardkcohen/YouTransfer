...
module.exports = (function() {
	...
	self.download = function(token, req, res, next) {
		self.storageFactory.get(function(err, factory) {
			factory.download(token, req, res, next);
		});
	};

	self.archive = function(token, req, res, next) {
		self.storageFactory.get(function(err, factory) {
			factory.archive(token, req, res, next);
		});
	};
								...
								res.renderTemplate('message.html', fields, function(err, body) {
									if(!err) {
										var email = {
											...
											to: fields.email.to,
											subject: `${settings.email.sender} sent you a file`,
											html: body
										};
